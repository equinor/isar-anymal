...
class MissionStatusHandler:
    def __init__(self) -> None:
        self.last_mission_event: MissionEventDto | None = None
    ...
    def get_task_status(self, task_id: str) -> TaskStatus:
        task_status: ANYmalTaskStatus | None = None
        task_outcome: Outcome | None = None
    ...
    def get_mission_status(self) -> MissionStatus:
        ...
        try:
            task_summaries: list[TaskSummaryDto] = self._get_anymal_task_summaries()
            task_statuses = [
                self._anymal_task_summary_to_task_status(task.status, task.outcome)
        ...
        if all(status == TaskStatus.NotStarted for status in task_statuses):
            return MissionStatus.NotStarted
        if any(
            ...
            return MissionStatus.InProgress
        if all(status == TaskStatus.Successful for status in task_statuses):
            return MissionStatus.Successful
        if all(status == TaskStatus.Failed for status in task_statuses):
